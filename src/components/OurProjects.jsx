...
export default function ProjectsCarousel() {
  ...
  useEffect(() => {
    ...
    const hasVideo = PROJECTS[index]?.video || PROJECTS[index]?.youtubeUrl;
    if (hasVideo) return; // Pause auto-scroll if current project has a video
  ...
  const resumeAuto = () => {
    pauseAuto();
    const hasVideo = PROJECTS[index]?.video || PROJECTS[index]?.youtubeUrl;
    if (hasVideo) return; // Don't resume auto-scroll if on a video
    autoRef.current = setInterval(() => {
      setIndex((prev) => {
        const next = (prev + 1) % PROJECTS.length;
        centerItem(next);
  ...
  useEffect(() => {
    const video = PROJECTS[index]?.video ? videoRefs.current[index] : null;
    ...
    <Box
      ...
      sx={{
        ...
        "&::before": {
          ...
          height: "140%",
          background: `
            radial-gradient(circle at 15% 50%, rgba(99, 102, 241, 0.08) 0%, transparent 35%),
        ...
        "@keyframes meshFade": {
          ...
          "100%": { opacity: 0.4, transform: "scale(1.1) rotate(-2deg)" }
        }
      }}
            ...
            >
              <Box
                sx={{
                  ...
                  px: {
                    xs: 0,
                    md: perView === 3 ? "10%" : perView === 2 ? "20%" : "7.5%"
                  },
                  alignItems: "center",
                }}
              >
                {PROJECTS.map((p, i) => {
                  const isYouTube = Boolean(p.youtubeUrl);
                  const dist = circularDistance(i, index, PROJECTS.length);
                  const opacity = dist === 0 ? 1 : dist === 1 ? 0.6 : 0.3;
                  const isActive = dist === 0;
                  const currentTilt = tilt.id === i ? `perspective(1000px) rotateX(${tilt.y}deg) rotateY(${tilt.x}deg)` : "perspective(1000px) rotateX(0deg) rotateY(0deg)";
                  const transform = isActive
                    ? (perView === 1 ? "translateY(-10px)" : `scale(1.1) translateY(-10px) ${currentTilt}`)
                    : (perView === 1 ? "scale(1)" : `scale(0.95) ${currentTilt}`);
                  ...
                  return (
                    <Box
                      ...
                      sx={{
                        ...
                        minWidth: {
                          xs: "100%",
                          sm: perView === 1 ? "85%" : perView === 2 ? "60%" : "80%",
                          md: perView === 3 ? "80%" : perView === 2 ? "60%" : "85%",
                        },
                        height: { xs: "auto", sm: "350px", md: 450 },
                        minHeight: { xs: "auto", sm: "350px", md: 450 },
                        ...
                        flex: "0 0 auto",
                        boxShadow: isActive
                          ? "0 30px 60px rgba(15,23,42,0.25), 0 0 20px rgba(99, 102, 241, 0.15)"
                          : "0 8px 20px rgba(15,23,42,0.1)",
                        border: isActive ? "2px solid rgba(99, 102, 241, 0.5)" : "2px solid rgba(255,255,255,0.05)",
                        transition: "transform 800ms cubic-bezier(0.2, 0.8, 0.2, 1), box-shadow 800ms cubic-bezier(0.4, 0, 0.2, 1), opacity 800ms ease",
                        ...
                        cursor: "pointer",
                        backgroundColor: "#fff",
                        animation: isActive ? "activeFloat 6s ease-in-out infinite alternate" : "none",
                        "@keyframes activeFloat": {
                          "0%": { transform: transform },
                          "100%": { transform: transform + " translateY(-4px)" }
                        },
                        "&:hover": {
                          opacity: 1,
                          transform: isActive ? transform + " scale(1.01)" : "scale(1) translateY(-8px)",
                          boxShadow: "0 40px 80px rgba(15,23,42,0.3)",
                          "& .tech-stack-badges": { opacity: { xs: 0, md: 1 }, transform: "translateX(0)" }
                        },
                        "&:hover .video-controls-overlay": { opacity: 1, pointerEvents: "auto" },
                        "&:hover .video-info-overlay": { opacity: 1 },
                        "&:hover .static-title-overlay": { opacity: 0 },
                        "&:hover .static-category-overlay": { opacity: 0 }
                      }}
                      onMouseMove={(e) => {
                        if (perView === 1) return;
                      ...
                      <Box sx={{ display: "flex", flexDirection: "column", height: "100%", width: "100%" }}>
                        {p.youtubeUrl ? (
                          <Box sx={{
                            ...
                            display: "flex",
                            borderRadius: { xs: "inherit", md: "inherit" },
                            flexShrink: 0
                          }}>
                            <Box
                              component="iframe"
                            ...
                            {/* Desktop hover info (non-blocking for controls) */}
                            <Box
                              ...
                              sx={{
                                ...
                                pt: 8,
                                px: 3,
                                background: "linear-gradient(180deg, rgba(15, 23, 42, 0.95) 0%, rgba(15, 23, 42, 0.4) 45%, rgba(15, 23, 42, 0) 100%)",
                                color: "white",
                                opacity: 0,
                                pointerEvents: "none",
                        ...
                        >
                          {p.tech?.map((t, idx) => (
                            <Box key={idx} sx={{
                              ...
                              "&:hover": { transform: "scale(1.1)", bgcolor: "rgba(255, 255, 255, 0.25)" }
                            }}>
                              {t}
                            </Box>
                          ))}
                        ...
                        <Box sx={{
                          ...
                          className="static-category-overlay"
                        >
                          {p.category}
                        </Box>
                        ...
                        <Box sx={{
                          ...
                          className="static-title-overlay"
                        >
